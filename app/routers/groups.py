...
from app.dependencies.dependencies import get_session, get_current_user_with_activity, set_user_status, entity_activity_check
from app.dependencies.permissions_validators import check_permission

router = APIRouter(prefix="/groups", tags=["Groups"], dependencies=[Depends(set_user_status(True))])

# Создание группы
@router.post("/", response_model=group_schemas.GroupOutFull, status_code=status.HTTP_201_CREATED)
async def create_group(group_data: group_schemas.GroupCreate, 
...
@router.post("/{group_id}/add_member/", status_code=status.HTTP_200_OK, 
             dependencies=[Depends(check_permission(2)),
                           Depends(entity_activity_check(id_field="group_id", entity=Groups))])
async def add_member(group_id: int, member_data: group_schemas.GroupAddMember,
                     current_user: Users=Depends(get_current_user_with_activity), db: AsyncSession=Depends(get_session)):
...
@router.put("/{group_id}/", response_model=group_schemas.GroupOutFull, 
            ...
            dependencies=[Depends(check_permission(3)),
                          Depends(entity_activity_check(id_field="group_id", entity=Groups))])
async def update_group(group_id: int, group_data: group_schemas.GroupUpdate, 
                       current_user: Users=Depends(get_current_user_with_activity), db: AsyncSession=Depends(get_session)):
...
@router.delete("/{group_id}/", status_code=status.HTTP_200_OK, 
               dependencies=[Depends(check_permission(3)),
                             Depends(entity_activity_check(id_field="group_id", entity=Groups))])
async def delete_group(group_id: int, current_user: Users=Depends(get_current_user_with_activity), db: AsyncSession=Depends(get_session)):
